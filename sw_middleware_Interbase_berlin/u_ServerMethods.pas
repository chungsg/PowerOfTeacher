...
begin
  ...
  qry_public.Close;
  qry_public.SQL.Clear;
  qry_public.Sql.Add('select username from member where grade = :graqde and ban = :ban order by bunho asc');
  qry_public.Params[0].AsString := grade;
  qry_public.Params[1].AsString := ban;
  ...
  for i := 0 to qry_public.RecordCount -1 do
  begin
    NameAll := NameAll + qry_public.fieldbyname('username').AsString + '|';
    qry_public.Next;
  end;
...
begin
  qry_public.Close;
  qry_public.SQL.Clear;
  qry_public.Sql.Add('select * from currenturl where grade = :grade and ban = :ban');
  qry_public.Params[0].AsString := grade;
  qry_public.Params[1].AsString := ban;
  qry_public.Open;
...
begin
  qry_public.Close;
  qry_public.SQL.Clear;
  qry_public.Sql.Add('select * from currenturl order by grade asc');
  qry_public.Open;
  ...
  begin
    ...
    for i := 0 to qry_public.RecordCount - 1 do
    begin
      url := url + qry_public.FieldByName('grade').AsString + '|' + qry_public.FieldByName('ban').AsString + '|' +
      qry_public.FieldByName('url').AsString + '|';
      qry_public.Next;
    end;
  end;
  ...
  begin
    ...
    begin
      ...
      begin
        qry_public.Close;
        qry_public.SQL.Clear;
        qry_public.Sql.Add('update currenturl set url = :videoroom where grade = :grade and ban = :ban');
        qry_public.Params[0].AsString := videoroom;
        qry_public.Params[1].AsString := grade;
        qry_public.Params[2].AsString := ban;
        qry_public.ExecSQL;
      end else
      begin
        qry_public.Close;
        qry_public.SQL.Clear;
        qry_public.Sql.Add('insert into currenturl(grade, ban, url) values(:grade, :ban, :videoroom)');
        qry_public.Params[0].AsString := grade;
        qry_public.Params[1].AsString := ban;
        qry_public.Params[2].AsString := videoroom;
        qry_public.ExecSQL;
      end;
    end;
    FreeAndNil(UrlList);
...
begin
  qry_public.Close;
  qry_public.SQL.Clear;
  qry_public.Sql.Add('select * from swmaterials where (grade = :grade) and (lesson = :lesson)');
  qry_public.ParamByName('grade').AsString := grade;
  qry_public.ParamByName('lesson').AsString := lesson;
  qry_public.Open;
  ...
  begin
    ...
    for i := 0 to Qry_public.RecordCount - 1 do
    begin
      rtnData := rtnData + qry_public.FieldByName('pattern').AsString + '|' + qry_public.FieldByName('mname').AsString +'|'+
             qry_public.FieldByName('filename').AsString + '|' + qry_public.FieldByName('comment').AsString + '|';
      qry_public.Next;
    end;
  end;
...
begin
  qry_public.Close;
  qry_public.SQL.Clear;
  qry_public.Sql.Add('select * from swmaterials where mname like "%' + Name +'%"');
  qry_public.Open;
...
begin
  qry_public.Close;
  qry_public.SQL.Clear;
  qry_public.Sql.Add('insert into swmaterials(grade, lesson, pattern, mname, filename, comment) values(' +
                     ':grade, :lesson, :pattern, :mname, :filename, :comment)');
  qry_public.ParamByName('grade').AsString := grade;
  qry_public.ParamByName('lesson').AsString := lesson;
  qry_public.ParamByName('pattern').AsString := pattern;
  qry_public.ParamByName('mname').AsString := mname;
  qry_public.ParamByName('filename').AsString := filename;
  qry_public.ParamByName('comment').AsString := comment;
  qry_public.ExecSQL;

  qry_public.Close;
  qry_public.SQL.Clear;
  qry_public.Sql.Add('select * from swmaterials where (pattern = :pattern) and (mname = :mname) and (filename = :filename)');
  qry_public.Params[0].AsString := pattern;
  qry_public.Params[1].AsString := mname;
  qry_public.Params[2].AsString := filename;

  qry_public.Open();

  if qry_public.RecordCount = 0 then result := false else result := true;
end;
...
begin
  qry_public.Close;
  qry_public.SQL.Clear;
  qry_public.Sql.Add('select * from swteaching where (tname = :tname) and (lesson = :lesson)');
  qry_public.ParamByName('tname').AsString := tname;
  qry_public.ParamByName('lesson').AsString := lesson;
  qry_public.Open;
  ...
  begin
    ...
    for i := 0 to Qry_public.RecordCount - 1 do
    begin
      rtnData := rtnData + qry_public.FieldByName('lname').AsString + '^' +
        qry_public.FieldByName('contents').AsString + '^';
      qry_public.Next;
    end;
  end;
  if rtnData <> '' then result := rtnData;
...
begin
  qry_public.Close;
  qry_public.SQL.Clear;
  qry_public.Sql.Add('select * from swteaching where (tname = :tname) and  (lesson = :lesson) and (lname = :lname) ');
  qry_public.Params[0].AsString := tname;
  qry_public.Params[1].AsString := lesson;
  qry_public.Params[2].AsString := lname;
  qry_public.Open();

  if qry_public.RecordCount > 0 then
  begin
    qry_public.First;
    RegiNum := qry_public.FieldByName('num').AsInteger;

    qry_public.Close;
    qry_public.SQL.Clear;
    qry_public.Sql.Add('update swteaching set contents = :contents where num = :reginum');
    qry_public.Params[0].AsString := contents;
    qry_public.Params[1].AsInteger := reginum;
    qry_public.ExecSQL;
  end else
  begin
    qry_public.Close;
    qry_public.SQL.Clear;
    qry_public.Sql.Add('insert into swteaching(tname, lesson, lname, contents) values(' +
                       ':tname, :lesson, :lname, :contents)');
    qry_public.ParamByName('tname').AsString := tname;
    qry_public.ParamByName('lesson').AsString := lesson;
    qry_public.ParamByName('lname').AsString := lname;
    qry_public.ParamByName('contents').AsString := contents;
    qry_public.ExecSQL;
  end;
  ...
  qry_public.Close;
  qry_public.SQL.Clear;
  qry_public.Sql.Add('select * from swteaching where (lesson = :lesson) and (lname = :lname)');
  qry_public.Params[0].AsString := lesson;
  qry_public.Params[1].AsString := lname;
