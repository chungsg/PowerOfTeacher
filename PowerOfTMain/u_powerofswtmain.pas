...
uses
  ...
  IdMultipartFormData, iexUserInteractions, imageenio, imageenproc, iemio,
  iemview, iexFolderMView, System.Actions, Vcl.ActnList;
...
//---------------------------------------------------------------------------
 ...
 //---------------------------------------------------------------------------
  ...
  Tfm_mainteacher = class(TForm)
    ...
    procedure Btn_WhatisthisClick(Sender: TObject);
    procedure Btn_YoutubeClick(Sender: TObject);
  ...
  private
    ...
    procedure ShowPage(pc: TPageControl; ShowPage: TTabSheet);
...
var
  ...
  tname, tid, tgrade, tban, tbunho, tiphak, tconnectip : String;
...
procedure Tfm_mainteacher.ShowPage(pc: TPageControl; ShowPage: TTabSheet);
var
  i: Integer;
begin
  for i := 0 to TPageControl(PC).ControlCount - 1 do
    TTabSheet(TPageControl(PC).Controls[i]).TabVisible := False;

    TPageControl(PC).ActivePage := ShowPage;
    ShowPage.TabVisible := True;
end;
...
//       Showmessage(Login);
        ...
        begin
          ...
          begin
            if tgrade = '1' then
            begin
                   tconnectip := '210.218.83.98';
                   SQLConnection1.Params.Values['HostName'] := tconnectip;
                   DSClientCallbackChannelManager1.DSHostname := tconnectip;
                   subpath := 'first';
            end;
            if tgrade = '2' then
            begin
...
procedure Tfm_mainteacher.Btn_WhatisthisClick(Sender: TObject);
begin
  if tname = '' then
  begin
    btn_login.Tag := 6;
  ...
  begin
    ShellExecute(handle, 'open','whatisthis.exe',PChar(tid+'|'+tname+'|'+tgrade+'|'+tban+'|'+tbunho+'|'+tiphak+'|'+tconnectip),nil,SW_NORMAL);
    Application.Terminate;
  end;
end;

procedure Tfm_mainteacher.Btn_YoutubeClick(Sender: TObject);
begin
  if tname = '' then
  begin
    btn_login.Tag := 6;
  ...
  begin
    ShellExecute(handle, 'open','NonCommercial.exe',PChar(tid+'|'+tname+'|'+tgrade+'|'+tban+'|'+tbunho+'|'+tiphak+'|'+tconnectip),nil,SW_NORMAL);
    Application.Terminate;
  end;
end;
...
begin
  if ParamStr(1) = '' then
  begin
    Showmessage('PowerOfTeacher.EXE를 더블클릭하여 프로그램을 실행시키시기 바랍니다.');
    Application.Terminate;
  end else
  begin
    Person := TStringList.Create;
